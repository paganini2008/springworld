...
public abstract class SqlScripts {
	...
	public static final String DEF_DDL_JOB_DETAIL = "create table cluster_job_detail(job_id int primary key auto_increment, signature varchar(255), job_name varchar(255) not null, job_class_name varchar(255) not null, group_name varchar(45) not null, description varchar(255), attachment varchar(255), create_date timestamp)";
	public static final String DEF_DDL_JOB_TRIGGER = "create table cluster_job_trigger(job_id int not null, trigger_type int not null, trigger_description varchar(255) not null)";
	...
	public static final String DEF_INSERT_JOB_DETAIL = "insert into cluster_job_detail(signature,job_name,job_class_name,group_name,description,attachment,create_date) values (?,?,?,?,?,?,?)";
	...
	public static final String DEF_UPDATE_JOB_RUNTIME_START = "update cluster_job_runtime set job_state=?, last_execution_time=?, next_execution_time=? where job_id=(select id from cluster_job_detail where job_name=? and job_class_name=?)";
	...
	public static final String DEF_UPDATE_JOB_STATE = "update cluster_job_runtime set job_state=? where job_name=? and job_class_name=?";
	...
	public static final String DEF_SELECT_JOB_STAT = "select job_id, sum(complete) as completeCount, sum(failed) as failedCount, sum(skipped) as skippedCount from cluster_job_trace where job_id=(select job_id from cluster_job_detail where job_name=? and job_class_name=?) group by job_id";
