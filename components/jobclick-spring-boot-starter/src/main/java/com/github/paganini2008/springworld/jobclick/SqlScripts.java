...
public abstract class SqlScripts {
	...
	public static final String DEF_INSERT_JOB_DETAIL = "insert into ck_job_detail(cluster_name,group_name,job_name,job_class_name,description,attachment,email,retries,weight,create_date) values (?,?,?,?,?,?,?,?,?,?)";
	...
	public static final String DEF_INSERT_JOB_TRIGGER = "insert into ck_job_trigger(job_id, trigger_type, trigger_description, start_date, end_date, repeat_count) values (?,?,?,?,?,?)";
	public static final String DEF_INSERT_JOB_DEPENDENCY = "insert into ck_job_dependency(job_id, dependent_job_id) value (?,?)";

	public static final String DEF_UPDATE_JOB_DETAIL = "update ck_job_detail set description=?, attachment=?, email=?, retries=?, weight=? where cluster_name=? and group_name=? and job_name=? and job_class_name=?";
	public static final String DEF_UPDATE_JOB_TRIGGER = "update ck_job_trigger set trigger_type=?, trigger_description=?, start_date=?, end_date=?, repeat_count=? where job_id=?";
	...
	public static final String DEF_SELECT_JOB_RELATIONS = "select * from ck_job_detail where job_id in (select job_id from ck_job_dependency where dependent_job_id=?)";

	public static final String DEF_SELECT_JOB_TRACE = "select * from ck_job_trace where job_id=? and execution_time between ? and ?";
	public static final String DEF_SELECT_JOB_INFO = "select a.*,b.job_state,b.last_running_state,b.last_execution_time,b.last_completion_time,b.next_execution_time,c.trigger_type,c.trigger_description,c.start_date,c.end_date from ck_job_detail a join ck_job_runtime b on b.job_id=a.job_id join ck_job_trigger c on c.job_id=b.job_id where a.cluster_name=? order by create_date desc";
